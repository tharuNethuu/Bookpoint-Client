...
const OrdersTable = () => {
  ...
  useEffect(() => {
    fetch('https://bookpoint-client.vercel.app/orders')
      .then(response => {
        if (!response.ok) {
  ...
  const handleDelete = async (orderId) => {
    const response = await fetch(`https://bookpoint-client.vercel.app/orders/${orderId}`, {
      method: 'DELETE'
    });
...
const handleAssignPersonChange = async (orderId, assignedPerson) => {
  const response = await fetch(`https://bookpoint-client.vercel.app/orders/${orderId}/assign`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json'
    },
